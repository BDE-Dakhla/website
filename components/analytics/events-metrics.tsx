...
export function AnalyticsEvents({ range }: { range: Range }) {
  // Clamp to <= 12h
  const effectiveRange: Range = (range === '24h' || range === '7d' || range === '30d' || range === '90d' || range === '6mo' || range === '1y') ? '12h' : range

  const { data } = useSWR<{ items: SummaryItem[]; series: SeriesRow[] }>(`/api/insights/events?range=${effectiveRange}`, fetcher, {
  ...
  const { seriesData, eventNames } = useMemo(() => {
    ...
    for (const r of (data?.series ?? [])) {
      const ts = new Date(r.bucket as any)
      const key = ts.toISOString()
      names.add(r.event_name)
      if (!byBucket[key]) byBucket[key] = { ts, label: formatHourLabel(ts) }
      byBucket[key][r.event_name] = r.c
    }
    const arr = Object.values(byBucket).sort((a: any, b: any) => a.ts.getTime() - b.ts.getTime())
    return { seriesData: arr as any[], eventNames: Array.from(names) }
  }, [data])
            ...
            <div className="space-y-2">
              {items.map((it) => (
                <div key={it.name} className="flex items-center gap-3">
                  ...
                  <div className="w-16 text-right text-muted-foreground text-sm tabular-nums">{it.count.toLocaleString()}</div>
                  <div className="ms-2 w-10 text-right text-muted-foreground text-xs">{it.percent}%</div>
                  <div className="ms-3 h-2 flex-1 rounded bg-muted">
                    <div className="h-2 rounded bg-primary" style={{ width: `${it.percent}%` }} />
                  </div>
                </div>
              ))}
              {items.length === 0 && <div className="text-muted-foreground text-sm">No data</div>}
            </div>
            <div className="h-[260px] md:col-span-3">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={seriesData} margin={{ left: 8, right: 8, top: 8 }}>
                  ...
                  {eventNames.map((name, i) => (
                    <Bar key={name} dataKey={name} stackId="events" fill={colorFor(i)} />
                  ))}
                </BarChart>
              </ResponsiveContainer>
            </div>
