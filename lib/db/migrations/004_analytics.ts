...
export async function up(db: Kysely<Database>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('visitor_id', 'uuid', (col) =>
      col.notNull().references('analytics_visitors.id').onDelete('cascade'),
    )
  ...
  await db.schema
    ...
    .addColumn('session_id', 'uuid', (col) =>
      col.notNull().references('analytics_sessions.id').onDelete('cascade'),
    )
    .addColumn('happened_at', 'timestamptz', (col) =>
      col.notNull().defaultTo(sql`now()`),
    )
...
export async function down(db: Kysely<Database>): Promise<void> {
  ...
  await db.schema
    .dropIndex('analytics_sessions_last_activity_idx')
    .ifExists()
    .execute()
