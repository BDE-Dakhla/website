...
export function BigCalendar(props: Props) {
  ...
  const [month, setMonthState] = useState<Date>(new Date(initial.getFullYear(), initial.getMonth(), 1))
  ...
  const goToday = () => setMonthState(new Date(today.getFullYear(), today.getMonth(), 1))
    ...
    <div className='flex min-h-0 flex-1 grow flex-col overflow-hidden'>
      <div className='flex shrink-0 items-center justify-between px-2 pb-2'>
        <div className='flex items-center gap-2'>
          <Button aria-label='Previous month' onClick={goPrev} size='icon' variant='outline'>
            <ChevronLeft className='h-4 w-4' />
          </Button>
          <Button aria-label='Next month' onClick={goNext} size='icon' variant='outline'>
